...
function transformCustomerService(
  service: BackendCustomerService,
  customerId: number,
): CustomerService {
...
function flattenServices(
  services: BackendCustomerService[],
  customerId: number,
): CustomerService[] {
...
export const notificationsApi = {
  ...
  listAll: async (
    params: Omit<NotificationsListParams, 'page' | 'pageSize'> = {},
  ...
  triggerServiceEnding: async (): Promise<{
    created: number;
    skipped: number;
    auto_dismissed: number;
  }> => {
    const response = await fetchWithAuth(`/notifications/generate_service_ending/`, {
      method: 'POST',
    });
    return response.json();
  },
